...
# ===== 使用严格的 prompt 避免 VLM 输出格式问题 =====
# 在 import LocalRagService 之前替换 PROMPTS，避免 VLM 输出额外文本导致 regex fallback
from raganything.prompt import PROMPTS
from raganything.prompt_qwen2_vl import PROMPTS as PROMPTS_QWEN2VL
PROMPTS.update(PROMPTS_QWEN2VL)
# =====================================================
...
# API 配置
RAG_API_BASE = "http://localhost:8001/v1"      # Qwen2-VL-7B（生成答案）
JUDGE_API_BASE = "http://localhost:8002/v1"    # Qwen2.5-32B（评估）

RAG_MODEL_NAME = "Qwen/Qwen2-VL-7B-Instruct"
...
async def generate_answers(start_id: int = 0, end_id: int = 229, resume: bool = True):
    """
    为 DocBench 生成系统答案
    
    前提：Qwen2-VL-7B 服务已在 8001 端口运行
    ...
    # 【简化配置】完全使用默认配置，不做任何修改（像 raganything_local_v2.py 一样）
    settings = LocalRagSettings.from_env()
    # 不修改 working_dir_root 和 output_dir，让它使用默认值
    ...
    with open(output_file, 'a', encoding='utf-8') as f_out:
        for doc_id in range(start_id, end_id):
            ...
            try:
                ...
                doc_output_dir = f"./output/docbench_{doc_name}"
...
Examples:
  ...
  tail -f run.log
    ...
    if args.mode == 'generate':
        logger.info("⚠️  Please ensure Qwen2-VL-7B is running on port 8001")
